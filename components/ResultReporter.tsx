...
import { CheckResults, FileResults, PackageResult } from '../lib/types'
...
const filterNoMatch = (results, initialConfig: Config) => {
  ...
  const allFilesRules = [].concat.apply(
    [],
    results.map((file) => file.rules.map(({ id }) => id))
  )
...
export function ResultsCompare({ results, currentResults, outputHtml }) {
  const tableResults = Object.keys(results)
    .map((fileName) => {
...
export function ResultsPackages({ results }: { results: CheckResults }) {
  const list = (results.results as PackageResult[]).map((result) => (
    <Box key={result.package}>
      <Text bold color="red" underline>
        {result.package}
      </Text>
