...
function Walk({ config, include = null, openReport = false }) {
  ...
  useEffect(() => {
    (async () => {
      if (isConfigValid && isGitReady && !isHistoryDirty) {
        ...
        const walkResults = await walkCommits<RevisionResults, WalkLoopResult>(
          revList.reverse(),
          {
